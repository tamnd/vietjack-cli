...
describes what users get. Pick a format with `-o`, or let vietjack choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
vietjack <command> -o table   # aligned columns for reading
vietjack <command> -o jsonl   # one JSON object per line, for piping
vietjack <command> -o json    # a single JSON array
vietjack <command> -o csv     # spreadsheet friendly
vietjack <command> -o tsv     # tab-separated
vietjack <command> -o url     # just the URL column
vietjack <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
vietjack <command> --fields id,title,url
```
...
```bash
vietjack <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
vietjack <command>            # a table, because this is a terminal
vietjack <command> | wc -l    # JSONL, because this is a pipe
```
